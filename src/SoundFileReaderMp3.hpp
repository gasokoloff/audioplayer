#ifndef SOUND_FILE_READER_MP3
#define SOUND_FILE_READER_MP3
...
protected:
  ...
  public:
    ...
    bool update(mpg123_handle *handle) {
      return mpg123_getformat(handle, &m_rate, &m_channels, &m_encoding) ==
             MPG123_OK;
    }
  ...
  public:
    ...
    std::uint32_t getSamplingRateInHz() const { return m_info.rate; }
    ...
    std::string toString() const {
      ...
      switch (m_info.mode) // Only the mono mode has 1 channel, the others have
                           // 2 channels.
      {
      case mpg123_mode::MPG123_M_STEREO:
      ...
      result += "\nheader: MPEG Audio Layer (MP1/MP2/MP3): " +
                std::to_string(m_info.layer);
      result += "\nheader: Mode extension bit flag: " +
                std::to_string(m_info.mode_ext);
      result +=
          "\nheader: Emphasis type (?): " + std::to_string(m_info.emphasis);
      result += "\nheader: Target average bitrate: " +
                std::to_string(m_info.abr_rate);
      result += "\nheader: Bitrate of the frame (kbps): " +
                std::to_string(m_info.bitrate);
      result += "\nheader: Sampling rate in Hz: " + std::to_string(m_info.rate);
      result += "\nheader: Size of the frame (in bytes, including header): " +
                std::to_string(m_info.framesize);
