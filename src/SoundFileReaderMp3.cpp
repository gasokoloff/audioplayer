...
SoundFileReaderMp3::SoundFileReaderMp3()
    : m_handle(nullptr), m_stream(nullptr), m_frameInfo(), m_outputFormat(),
      m_streambufferSize(80000) // any size will work
      ,
      m_streambuffer(new unsigned char[m_streambufferSize]) {}

SoundFileReaderMp3::~SoundFileReaderMp3() { close(); }
...
bool SoundFileReaderMp3::open(sf::InputStream &stream,
  ...
  std::cout << "wrote info: channels: " << info.channelCount
            << ", expected number of samples: " << info.sampleCount
...
bool SoundFileReaderMp3::initializeLibrary() {
  ...
  if (!m_handle) {
    std::cerr << "Unable to create mpg123 handle: " << mpg123_plain_strerror(ok)
              << std::endl;
    return false;
  }
  std::cout << "created handle at address " << (std::uintptr_t)m_handle
            << std::endl;

  // Later we have to read the audio input from sf::InputStream, which can be
  // from a file, from the network or from any other input stream. This decides
  // how we open mpg123:
  ok = mpg123_open_feed(m_handle);
  if (ok != MPG123_OK) {
    std::cerr << "mpg123_open_feed(): " << mpg123_plain_strerror(ok)
              << std::endl;
    return false;
  }
...
bool SoundFileReaderMp3::probeFirstFrame(sf::InputStream &stream) {
  // select a rather small buffer size. we don't want to read actual payload
  // data. we only want to read till the mp3 library can extract header
  // information
  ...
  for (bool needMore = true; needMore;) {
    sf::Int64 bytesReadFromStream =
        stream.read(streamBuffer, streamSize); // read from input stream
    numReadsFromStream++;
    if (bytesReadFromStream <= 0) {
      std::cout << (bytesReadFromStream == 0
                        ? "stream empty"
                        : ("stream error: " + bytesReadFromStream))
                << std::endl;
      break;
    }
    totalReadBytes += bytesReadFromStream;

    // feed until happy, do not touch decoded data. we only want to get header
    // information
    std::size_t numDecodedBytes = 0;
    int ret = mpg123_decode(m_handle, streamBuffer, bytesReadFromStream,
                            nullptr, 0, &numDecodedBytes);
...
// \return Number of samples actually read. If less samples than maxCount are
// read, sfml assumes that EOF has been reached and terminates playback.
sf::Uint64 SoundFileReaderMp3::read(sf::Int16 *samples, sf::Uint64 maxCount) {
  std::cout << "READ: request up to " << maxCount
            << " samples. size of streambuffer: " << m_streambufferSize
            << " bytes" << std::endl;
  ...
  // This is not the main decoding loop but a clean-up loop that consumes any
  // input left from the previous call. This happens if the sample buffer was
  // almost full but the decoder needed more raw input. We read from the stream
  // again and feed everything to the decoder. The decoder is only allowed to
  // fill the small amount left in the sample buffer and has to keep the
  // remaining input. Now is the time to consume that remaining input.
  int ret = 0;
  do {
    std::size_t numDecodedBytes = 0;
    ret = mpg123_decode(
        m_handle,
        nullptr // raw input buffer: since we want to consume the remaining
                // input, we don't provide any new one
        ,
        ...
        ,
        maxBytes -
            offset // number of decoded bytes we want to get out: this
                   // calculation is why casting to byte size was a good idea
        ,
        &numDecodedBytes);     // this tells us the size of the decoded data. it
                               // will never exceed the size we requested in the
                               // above parameter
    offset += numDecodedBytes; // set sample offset for next call
    std::cout << "decoded " << numDecodedBytes
              << " bytes from leftover input. offset into samples: " << offset
              << " / " << maxBytes << std::endl;

  } // NEED_MORE is returned when no more input is available and the output
    // array is not full yet. not sure about ERR though. probably when output is
    // full.
  ...
  while (ret == MPG123_NEED_MORE && ret != MPG123_ERR && offset < maxBytes) {
    ...
    sf::Int64 bytesReadFromStream =
        m_stream->read(m_streambuffer, m_streambufferSize);
    std::cout << "read " << bytesReadFromStream << " bytes from stream"
              << std::endl;
    if (bytesReadFromStream <= 0) {
      std::cout << (bytesReadFromStream == 0
                        ? "stream empty"
    ...
    std::size_t numDecodedBytes = 0;
    ret = mpg123_decode(m_handle, m_streambuffer, bytesReadFromStream,
                        &byteSamples[offset], maxBytes - offset,
                        &numDecodedBytes);
    offset += numDecodedBytes;
    std::cout << "decoded " << bytesReadFromStream << " to " << numDecodedBytes
              << " decoded bytes. total decoded bytes: " << offset << " / "
              << maxBytes << std::endl;
  }

  std::size_t totalSamples = offset / sizeof(short);

  std::cout << "samples delivered: " << totalSamples << " of " << maxCount
            << " requested" << std::endl;
  std::cout << "total bytes read: " << totalReadBytes << std::endl;
...
void SoundFileReaderMp3::seek(sf::Uint64 sampleOffset) {
  // tschumacher: Based on this example:
  // https://github.com/gypified/libmpg123/blob/master/doc/examples/feedseek.c
  // tschumacher: Spent a lot of time to figure out how to make that working
  // with SFML tschumacher: This implementation does not protect you from going
  // out of bounds (seeking beyond music duration), seek() itself does never
  // fail, but when you go out of bounds, the read() will fail.
  ...
  seekOffset =
      static_cast<off_t>(sampleOffset) /
      m_outputFormat
          .m_channels; // tschumacher: You need to make m_channels accessible

  // Feed seek to sample offset
  while ((status = mpg123_feedseek(m_handle, seekOffset, SEEK_SET,
                                   &inputOffset)) == MPG123_NEED_MORE) {
    ...
    if (mpg123_feed(m_handle, m_streambuffer, static_cast<size_t>(read)) ==
        MPG123_ERR) {
      std::cerr << "seek() @mpg123_feed(): " << mpg123_strerror(m_handle)
                << std::endl;
      return;
    }
  }
  ...
  if (status == MPG123_ERR) {
    std::cerr << "seek() @mpg123_feedseek(): " << mpg123_strerror(m_handle)
              << std::endl;
    return;
  }
